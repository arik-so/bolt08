...
export default class TransmissionHandler {
	...
	public async send(message: Buffer): Promise<Buffer> {
		...
		await this.incrementSendingNonce();
	...
	public async receive(undelimitedBuffer: Buffer): Promise<Buffer> {
		...
		await this.incrementReceivingNonce();
	...
	private async incrementSendingNonce() {
		this.sendingNonce++;
		if (this.sendingNonce >= 1000) {
			this.sendingKey = await this.rotateKey(this.sendingKey);
			this.sendingNonce = 0;
		}
	}

	private async incrementReceivingNonce() {
		this.receivingNonce++;
		if (this.receivingNonce >= 1000) {
			this.receivingKey = await this.rotateKey(this.receivingKey);
			this.receivingNonce = 0;
		}
	}

	private async rotateKey(key: Buffer): Promise<Buffer> {
		const derivative = await Handshake.hkdf(this.chainingKey, key);
